#!/bin/bash
# 启动 Ackermann 仿真导航 + openTCS 桥接 + RS-485 底盘协议仿真
# 用法: ./scripts/launch/sim_ackermann_rs485.sh
...
mkdir -p "${LOG_DIR}"
LOG_FILE="${LOG_DIR}/rs485_$(date +%Y-%m-%d_%H-%M-%S).log"

echo "日志文件: ${LOG_FILE}"

# 清理残留进程，避免 Gazebo/ROS2/socat 僵尸进程导致启动失败
